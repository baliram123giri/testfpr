...
import { ApnApiSearch } from "../service";
const ApnData = () => {
    //state

    const { apnFormData } = useSelector(
        (state) => state?.apnReducer
    );
    //function
    const { isLoading, data, mutate } = useMutation(ApnApiSearch, {
        onError({ message }) {
    ...
    useEffect(() => {
        if (apnFormData) {
            mutate({ apn: apnFormData?.apn });
        }
    }, [apnFormData?.apn]);

    //if ApnData not return null
    if (!apnFormData?.apn) {
        return null;
    } else if (isLoading) {
